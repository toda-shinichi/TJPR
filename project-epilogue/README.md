...
遊戲內的「線索與談判籌碼」採跨回合持久帳本：模型只能新增劇情中實際取得的證據、情報、人脈或通行權，並在使用、交付、曝光或證偽時更新狀態。狀態抽屜可直接點選仍可用的項目，帶入自由行動欄；已交付或失效項目會保留紀錄，但不再注入後續敘事提示詞。
...
上游限制是**每分鐘 5 次、跨模型且跨使用者共用**。封閉測試時多位玩家同時
按下選項會一起撞上額度，因此需要全域排隊。

### 為什麼放在 Worker，而且用 Durable Object

- 前端的 `waitForRpmCooldown()` 只管**自己那個瀏覽器**，不同玩家各算各的。
- Worker 的 KV 限速是**每 IP** 的，不同玩家不同 IP，同樣擋不到共用額度。
- Worker 是所有玩家的唯一匯流點，排隊必須放這裡。
- KV 是最終一致性，兩個並發請求可能讀到同一個「下一個空檔」並都認為輪到
  自己。**Durable Object 單執行緒、天然序列化**，是唯一能正確發號的選擇。

### 設計

| 參數 | 值 | 說明 |
| --- | --- | --- |
| `QUEUE_MIN_INTERVAL_MS` | 16000 | 約 3.75 RPM，為 5 RPM 的滾動窗口保留緩衝 |
| `QUEUE_MAX_WAIT_MS` | 180000 | 佇列超過 3 分鐘就請玩家稍後再試，不無限排隊 |
| `QUEUE_MAX_TOTAL_WAIT_MS`（前端） | 180000 | 累積等待的第二道防線 |

**等待中的請求不預先佔用時段**，只回報還要等多久，由前端睡完再問。這樣不會
因為玩家關掉分頁而留下無人認領的空檔（不需要逾期回收機制）。代價是多個等待
者可能同時醒來搶同一格 —— DO 會序列化，只有一個拿到，其餘拿到新的短等待。
三人規模下這個取捨是划算的。

**刻意不長時間佔住連線**：那樣得先以 HTTP 200 開頭回應，上游後續的錯誤狀態碼
就再也傳不回前端，現有的「模型不可用／限流／拒絕」判別會全部失效。改為回
429 + 等待秒數，由前端睡完重試。

### 前端行為

排隊**不是失敗**，不計入模型嘗試次數，睡完重試**同一個模型**。等待期間
loading 會顯示「目前排在第 N 位，預計還需 M 秒」。未綁定 `RPM_QUEUE` 時
Worker 直接放行，不會因為缺少綁定就整體不可用。

**Worker 路徑不再呼叫 `waitForRpmCooldown()`** —— 與排隊器重複計算，實測會讓
三次嘗試白等 32 秒。GAS 備援路徑不經過 Worker，仍保留前端限速。

### 實測

三個玩家同時送出：一人直接放行取得內容，其餘兩人收到「第 1 位、還需 16 秒」；
16 秒後重試即成功。
