...
function generateDynamicTurnChapter(turnCount, choiceId, customInput, profile) {
  ...
  if (isShura) {
    if (turnCount === 2) {
      ...
      return {
        chapterTitle: '第 1 幕 第 ' + turnCount + ' 回：絕境破局 · 暴雨突圍與命運定契',
        ...
        statusPanel: {
          ...
          pageCode: 'P.00' + turnCount
        },
        choices: [
          { id: 'opt_' + turnCount + 'a', label: '[A] 攜手黑道：反手扣緊徐令謙的五指「二爺，帶我走，以後我就是你的人」', risk: 'high', hint: '正式踏入黑幫世界，與徐令謙生死與共' },
          { id: 'opt_' + turnCount + 'b', label: '[B] 轉向司法：用力甩開徐令謙，撲入韓正寰懷中「韓檢，隨身碟交給你，帶我離開這裡」', risk: 'medium', hint: '選擇體制與司法正義，與韓正寰開啟白日戀情' },
          { id: 'opt_' + turnCount + 'c', label: '[C] 雙面通吃：在兩人夾縫中按下暗道炸藥定時器「兩位，誰先帶我突圍，隨身碟就是誰的！」', risk: 'high', hint: '極致獨立大女主，將兩大勢力玩弄於股掌之間' }
        ]
      };
    }
  } else {
    ...
    if (turnCount === 2) {
      ...
      return {
        chapterTitle: '第 1 幕 第 ' + turnCount + ' 回：暗夜突圍 · 生死與共的終局定約',
        ...
        choices: [
          { id: 'choice_' + turnCount + 'a', label: '[A] 堅定握緊：緊緊扣住他的手指「有二爺在，我什麼都不怕，走吧」', risk: 'high', hint: '確定終身盟約，邁入第二幕' },
          { id: 'choice_' + turnCount + 'b', label: '[B] 舉槍相助：自他腰間抽出一柄備用手槍「誰要你保護？今晚我們一起殺出去」', risk: 'medium', hint: '展現強悍實力，雙強攜手突圍' },
          { id: 'choice_' + turnCount + 'c', label: '[C] 終極破局：將隨身碟扔在桌上設定自毀定時「讓他們留在這裡替我們收尾，我們走！」', risk: 'high', hint: '引爆第一幕所有線索，瀟灑離去' }
        ]
      };
    }
  }
